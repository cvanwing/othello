...
//Author:       Christina Van Wingerden
//Revised:      5/3/18
...
public class VanWingerden {
    ...
    //===========================================
    //===========================================
    public static int methodOfPlay;
    public static int[][] cellValues = new int[8][8];
    //===========================================
    //===========================================
	//***************************************************************************************************
   	//Method:		main
	...
	public static void main(String args[]) {
        ...
        System.out.println("Player: Christina Van Wingerden\n");
        if (currentBoard != null) {                                 //board exhausted, make sure it can be used
        ...
        methodOfPlay = keyboardInput.getInteger(true, 3, 1, 3, "Select the method of play: "
                + "1 = intelligent, 2 = manual, 3 = random (default is random)");
        String playType = "";
        if(methodOfPlay == 1){
            playType = "intelligently";
        }
        if(methodOfPlay == 2){
            playType = "manually";
        }
        ...
        System.out.println("This player will play " + playType + "\n");
        ...
        while (currentBoard.status == 1) {
            if (currentBoard.whoseTurn == myColor) {
                ...
                Player myPlayer = new Player(myColor, opponentColor);
                ...
                //this sets myBoardObject to the MyBoard object that is chosen
                myBoardObject = myPlayer.chooseMove(currentBoard, methodOfPlay);
                
                myMove = myBoardObject.myMove;
                
                //make a deep copy of the current game board
//                boardCopy = copyBoard(currentBoard);
//                
//                //a balance so that the program is not too slow, but it can still look ahead enough to prune intelligently
//                int depth = 4;
//                //start the recursive DFS process of finding all the legal moves to a depth of 10
//                myMove = legalMove2(currentBoard, depth);
    ...
    public static Board copyBoard(Board currentBoard){
        Board boardCopy = new Board(currentBoard.boardRows, currentBoard.boardCols, currentBoard.maxMoveTime);
        
        for(int row = 0; row < 8; row++){
            for(int col = 0; col < 8; col++){
                boardCopy.board[row][col] = currentBoard.board[row][col];
            }
        }
        return boardCopy;
    } 
    ...
    //evaluation function
    //1. For later in the game: eval(state) = #tiles belonging to me − #tiles belonging to my opponent
    //2. in the beginning (minimalistic): eval(state) = #tiles belonging to opponent − #tiles belonging to me
    //3. based on priority of location: 
        // corners = highest priority
        // edges =  high priority
        // middle 4 spots = high priority (in beginning)
        // spot with most vacant neighbors = medium prority
        // one away from edge = low priority
        // spots surrounding a corner = lowest priority
	//***************************************************************************************************
}
//*******************************************************************************************************
//*******************************************************************************************************
...
//questions
//fix showBoard method
//do you suggest searching for an empty cell or a cell with an opponents color to make the legal move?